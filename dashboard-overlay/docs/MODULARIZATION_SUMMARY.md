...
|------|-------|--------------|---------|
| `config.js` | 240 | none | **Global constants & state** — SIMHUB_URL, POLL_MS, PROP_KEYS array, all state variables (_pollFrame, _currentGameId, _settings, etc.), demo models, manufacturer branding |
...
| File | Lines | Dependencies | Purpose |
|------|-------|--------------|---------|
| `poll-engine.js` | 896 | all core modules | **Main polling loop** — HTTP fetch from SimHub plugin, data→UI updates, connection status, game mode switching, asset loading, initialization of logo cycling, settings, Discord, and periodic polling at POLL_MS (33ms, ~30fps) |
...
| `pit-limiter.js` | 54 | poll-engine.js | Pit limiter indicator, speed limit display |
...
| File | Lines | Dependencies | Purpose |
|------|-------|--------------|---------|
| `webgl.js` | 1,099 | webgl-helpers.js, poll-engine.js | **WebGL FX engine** — Tachometer segment animations, pedal histogram bars, flag effects, leaderboard event animations, K10 logo effects; requires WebGL canvas contexts |

**Total JS:** 4,691 lines, 240KB
...
18. webgl.js              ← WebGL effects (must precede poll-engine)
19. poll-engine.js        ← Main loop (MUST be last; initializes everything)
```
...
4. **Script tags** — 19 JavaScript modules in dependency order
...
```
    ...
    ┌─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┐
    ↓     ↓     ↓     ↓     ↓     ↓     ↓     ↓     ↓     ↓
    lb   ds   rc   rt   inc  pit  re   form spt  fps  webgl
...
- The original `dashboard.html` (8,539 lines) is now `dashboard.html` (435 lines) + 27 modules.
