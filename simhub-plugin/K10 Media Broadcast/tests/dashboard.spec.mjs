/**
 * K10 Media Broadcast — Dashboard Test Suite
 *
...
test.describe('Track map', () => {
  test('track map updates when data arrives', async ({ page }) => {
    await loadDashboard(page, {
      'K10MediaCoach.Plugin.TrackMap.Ready': 1,
      'K10MediaCoach.Plugin.TrackMap.SvgPath': 'M10 10 L 90 10 L 90 90 L 10 90 Z',
      'K10MediaCoach.Plugin.TrackMap.PlayerX': 50,
      'K10MediaCoach.Plugin.TrackMap.PlayerY': 50,
      'K10MediaCoach.Plugin.TrackMap.Opponents': '30,30,0;70,70,0',
    });
  ...
  test('opponents render as circles', async ({ page }) => {
    await loadDashboard(page, {
      'K10MediaCoach.Plugin.TrackMap.Ready': 1,
      'K10MediaCoach.Plugin.TrackMap.SvgPath': 'M10 10 L 90 90',
      'K10MediaCoach.Plugin.TrackMap.PlayerX': 50,
      'K10MediaCoach.Plugin.TrackMap.PlayerY': 50,
      'K10MediaCoach.Plugin.TrackMap.Opponents': '20,20,0;80,80,0;50,10,1',
    });
...
test.describe('Connection backoff', () => {
  test('backoff counter increments on fetch failure', async ({ page }) => {
    // Route to fail
    await page.route(/k10mediacoach/, (route) => route.abort());
