...
This document specs out an automated visual regression testing tool for the K10 Media Coach SimHub dashboard. The tool captures screenshots of the dashboard at specific moments during a demo sequence and compares them against golden reference images to detect visual regressions.
...
- SimHub installed with the K10 Media Coach plugin and dashboard template
...
1. Find the SimHub dashboard window by title (the dashboard name is "k10 media coach")
...
def wait_for_step(expected_title, timeout=10):
    ...
    while time.time() - start < timeout:
        title = requests.get(f"{SIMHUB_URL}/K10MediaCoach.Plugin.CommentaryTitle").text.strip('"')
...
- Dashboard window must exist — SimHub needs to load the "k10 media coach" dashboard template
