...
using System.IO;
using System.Linq;
using K10MediaBroadcaster.Plugin.Models;
...
namespace K10MediaBroadcaster.Plugin.Engine
{
    ...
    public class CommentaryEngine
    {
        ...
        public void LoadTopics(string jsonPath)
        {
            ...
            {
                SimHub.Logging.Current.Warn($"[K10MediaBroadcaster] Topics file not found: {jsonPath}");
                LoadBuiltinTopics();
                return;
            }
            ...
            try
            {
                ...
                SimHub.Logging.Current.Info($"[K10MediaBroadcaster] Loaded {_topics.Count} topics from {jsonPath}");
            }
            catch (Exception ex)
            {
                SimHub.Logging.Current.Error($"[K10MediaBroadcaster] Failed to load topics: {ex.Message}");
                LoadBuiltinTopics();
            }
        }
        ...
        public void LoadSentiments(string jsonPath)
        {
            if (!File.Exists(jsonPath))
            {
                SimHub.Logging.Current.Warn($"[K10MediaBroadcaster] Sentiments file not found: {jsonPath}");
                return;
            }
            ...
            try
            {
                ...
                SimHub.Logging.Current.Info($"[K10MediaBroadcaster] Loaded {_sentimentColors.Count} sentiment colors");
            }
            catch (Exception ex)
            {
                SimHub.Logging.Current.Error($"[K10MediaBroadcaster] Failed to load sentiments: {ex.Message}");
            }
        }
            ...
            foreach (var topic in ordered)
            {
                ...
                if (IsVisible)
                {
                    ...
                    SimHub.Logging.Current.Info(
                        $"[K10MediaBroadcaster] Interrupting [{_currentTitle}] (sev {_currentSeverity}) with [{topic.Title}] (sev {topic.Severity})");
                }
                else
                {
        ...
        private void UpdateDemo()
        {
            ...
            if (step.IsInterrupt && IsVisible)
                SimHub.Logging.Current.Info(
                    $"[K10MediaBroadcaster][Demo] Interrupting [{_currentTitle}] (sev {_currentSeverity}) with [{topic.Title}] (sev {topic.Severity})");
        ...
        private void ShowPrompt(CommentaryTopic topic, TelemetrySnapshot context)
        {
            ...
            SimHub.Logging.Current.Info($"[K10MediaBroadcaster] Prompt shown: [{topic.Title}] (sev {topic.Severity}) {prompt}");
        }
