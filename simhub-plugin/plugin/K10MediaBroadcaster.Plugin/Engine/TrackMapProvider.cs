...
namespace K10MediaBroadcaster.Plugin.Engine
{
    ...
    public class TrackMapProvider
    {
        ...
        public void OnTrackChanged(string trackId)
        {
            ...
            {
                SimHub.Logging.Current.Info($"[K10MediaBroadcaster] Loaded track map for '{trackId}' from SimHub cache");
                return;
            }
            ...
            {
                SimHub.Logging.Current.Info($"[K10MediaBroadcaster] Loaded track map for '{trackId}' from K10 cache");
                return;
            }

            // No cached map — start recording
            _recording = true;
            SimHub.Logging.Current.Info($"[K10MediaBroadcaster] Recording track map for '{trackId}' — drive one clean lap");
        }
        ...
        private void FinaliseRecording()
        {
            ...
            {
                SimHub.Logging.Current.Warn("[K10MediaBroadcaster] Track recording too short, discarding");
            ...
            {
                SimHub.Logging.Current.Warn("[K10MediaBroadcaster] Track recording looks like a straight line (bad velocity data?), retrying next lap");
                _recording = true;
                _samples.Clear();
                return;
            }
        ...
        private bool TryParseMapFile(string filePath)
        {
            ...
            {
                SimHub.Logging.Current.Warn($"[K10MediaBroadcaster] Failed to parse map file '{filePath}': {ex.Message}");
                return false;
            }
        }
        ...
        private string GetOwnCacheDir()
        {
            return Path.Combine(
                ...
                "SimHub", "PluginsData", "K10MediaBroadcaster", "TrackMaps");
        }
        ...
        private void SaveToOwnCache(string trackId)
        {
            try
            {
                ...
                File.WriteAllText(path, sb.ToString());
                SimHub.Logging.Current.Info($"[K10MediaBroadcaster] Track map saved to {path}");
            }
            catch (Exception ex)
            {
                SimHub.Logging.Current.Warn($"[K10MediaBroadcaster] Failed to save track map: {ex.Message}");
            }
        }
