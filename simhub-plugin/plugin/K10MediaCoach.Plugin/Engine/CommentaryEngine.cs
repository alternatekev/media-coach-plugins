...
using System.IO;
using System.Linq;
using K10MediaCoach.Plugin.Models;
...
namespace K10MediaCoach.Plugin.Engine
{
    ...
    public class CommentaryEngine
    {
        ...
        public void LoadTopics(string jsonPath)
        {
            ...
            {
                SimHub.Logging.Current.Warn($"[K10MediaCoach] Topics file not found: {jsonPath}");
                LoadBuiltinTopics();
                return;
            }
            ...
            try
            {
                ...
                SimHub.Logging.Current.Info($"[K10MediaCoach] Loaded {_topics.Count} topics from {jsonPath}");
            }
            catch (Exception ex)
            {
                SimHub.Logging.Current.Error($"[K10MediaCoach] Failed to load topics: {ex.Message}");
                LoadBuiltinTopics();
            }
        }
        ...
        public void LoadSentiments(string jsonPath)
        {
            if (!File.Exists(jsonPath))
            {
                SimHub.Logging.Current.Warn($"[K10MediaCoach] Sentiments file not found: {jsonPath}");
                return;
            }
            ...
            try
            {
                ...
                SimHub.Logging.Current.Info($"[K10MediaCoach] Loaded {_sentimentColors.Count} sentiment colors");
            }
            catch (Exception ex)
            {
                SimHub.Logging.Current.Error($"[K10MediaCoach] Failed to load sentiments: {ex.Message}");
            }
        }
            ...
            foreach (var topic in ordered)
            {
                ...
                if (IsVisible)
                {
                    ...
                    SimHub.Logging.Current.Info(
                        $"[K10MediaCoach] Interrupting [{_currentTitle}] (sev {_currentSeverity}) with [{topic.Title}] (sev {topic.Severity})");
                }
                else
                {
        ...
        private void UpdateDemo()
        {
            ...
            if (step.IsInterrupt && IsVisible)
                SimHub.Logging.Current.Info(
                    $"[K10MediaCoach][Demo] Interrupting [{_currentTitle}] (sev {_currentSeverity}) with [{topic.Title}] (sev {topic.Severity})");
        ...
        private void ShowPrompt(CommentaryTopic topic, TelemetrySnapshot context)
        {
            ...
            SimHub.Logging.Current.Info($"[K10MediaCoach] Prompt shown: [{topic.Title}] (sev {topic.Severity}) {prompt}");
        }
