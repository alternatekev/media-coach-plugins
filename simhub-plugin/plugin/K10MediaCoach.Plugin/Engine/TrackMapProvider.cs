...
namespace K10MediaCoach.Plugin.Engine
{
    ...
    public class TrackMapProvider
    {
        ...
        public void OnTrackChanged(string trackId)
        {
            ...
            {
                SimHub.Logging.Current.Info($"[K10MediaCoach] Loaded track map for '{trackId}' from SimHub cache");
                return;
            }
            ...
            {
                SimHub.Logging.Current.Info($"[K10MediaCoach] Loaded track map for '{trackId}' from K10 cache");
                return;
            }

            // No cached map — start recording
            _recording = true;
            SimHub.Logging.Current.Info($"[K10MediaCoach] Recording track map for '{trackId}' — drive one clean lap");
        }
        ...
        private void FinaliseRecording()
        {
            ...
            {
                SimHub.Logging.Current.Warn("[K10MediaCoach] Track recording too short, discarding");
        ...
        private bool TryParseMapFile(string filePath)
        {
            ...
            {
                SimHub.Logging.Current.Warn($"[K10MediaCoach] Failed to parse map file '{filePath}': {ex.Message}");
                return false;
            }
        }
        ...
        private string GetOwnCacheDir()
        {
            return Path.Combine(
                ...
                "SimHub", "PluginsData", "K10MediaCoach", "TrackMaps");
        }
        ...
        private void SaveToOwnCache(string trackId)
        {
            try
            {
                ...
                SimHub.Logging.Current.Info($"[K10MediaCoach] Track map saved to {path}");
            }
            catch (Exception ex)
            {
                SimHub.Logging.Current.Warn($"[K10MediaCoach] Failed to save track map: {ex.Message}");
            }
        }
