...
namespace K10MediaCoach.Plugin.Engine
{
    ...
    public static class TriggerEvaluator
    {
        ...
        private static bool IsSpike(TriggerCondition t, TelemetrySnapshot cur, TelemetrySnapshot prev)
        {
            ...
            // (handles polling that misses the upswing but catches the peak + decline)
        ...
        {
            // TrackPositionPercent of -1 or extremely small delta can indicate off-track
            // SimHub doesn't directly expose PlayerTrackSurface, so we use a proxy:
            // large vertical acceleration spike often accompanies going off-track
            if (prev == null) return false;
        ...
        private static bool IsYellowFlag(TelemetrySnapshot cur, TelemetrySnapshot prev)
        {
            ...
            return curYellow && !prevYellow; // trigger on flag going active
        }
