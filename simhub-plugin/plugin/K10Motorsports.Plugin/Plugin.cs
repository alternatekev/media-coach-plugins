...
namespace K10Motorsports.Plugin
{
    ...
    public class Plugin : IPlugin, IDataPlugin, IWPFSettingsV2
    {
        ...
        public string LeftMenuTitle => "K10 Motorsports";
        ...
        {
            SimHub.Logging.Current.Info("[K10Motorsports] Initialising K10 Motorsports plugin");
            ...
            string feedbackPath = Path.Combine(
                ...
                "SimHub", "PluginsData", "K10Motorsports", "feedback.json");
...
// ── Events ────────────────────────────────────────────────────────
            ...
            SimHub.Logging.Current.Info("[K10Motorsports] Initialisation complete");
        }
        ...
        public void End(PluginManager pluginManager)
        {
            ...
            SimHub.Logging.Current.Info("[K10Motorsports] Plugin stopped, settings saved");
        }
        ...
        private void StartHttpServer()
        {
            ...
            foreach (var prefix in prefixes)
            {
                try
                {
                    ...
                    _httpListener.Start();
                    _httpThread = new Thread(HttpServerLoop) { IsBackground = true, Name = "K10Motorsports-HTTP" };
                    _httpThread.Start();
                    SimHub.Logging.Current.Info($"[K10Motorsports] HTTP state server listening on port 8889 (prefix: {prefix})");
                    return;
                }
                catch (Exception ex)
                {
            ...
            SimHub.Logging.Current.Warn("[K10Motorsports] HTTP server could not start on any prefix — dashboard overlay will not receive data");
        }
        ...
        private void HttpServerLoop()
        {
            ...
            while (_httpListener != null && _httpListener.IsListening)
            {
                ...
                try
                {
                        ...
                        try
                        {
                            ...
                            if (rw > 0.01 && rh > 0.01)
                            {
                                ...
                                SimHub.Logging.Current.Info(
                                    $"[K10Motorsports] Ambient rect set: x={rx:F3} y={ry:F3} w={rw:F3} h={rh:F3}");
                            }
                        }
                        catch (Exception ex)
                        {
                            SimHub.Logging.Current.Warn($"[K10Motorsports] setrect error: {ex.Message}");
                        }
                        byte[] okBytes = System.Text.Encoding.UTF8.GetBytes("{\"ok\":true}");
                        ctx.Response.ContentType = "application/json";
                        ctx.Response.Headers.Add("Access-Control-Allow-Origin", "*");
                    ...
                    // Pit stop selections
                    Jp(sb, "K10Motorsports.Plugin.PitBox.PitSvFlags", s.PitSvFlags);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.PitSvFuel", s.PitSvFuel, ic);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.PitSvLFP", s.PitSvLFP, ic);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.PitSvRFP", s.PitSvRFP, ic);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.PitSvLRP", s.PitSvLRP, ic);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.PitSvRRP", s.PitSvRRP, ic);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.TireCompound", s.PitSvTireCompound);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.FastRepair", s.PitSvFastRepair);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.Windshield", s.PitSvWindshieldTearoff);
                    // Pit box computed display strings
                    Jp(sb, "K10Motorsports.Plugin.PitBox.TireLF", s.PitTireLF ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.TireRF", s.PitTireRF ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.TireLR", s.PitTireLR ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.TireRR", s.PitTireRR ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.TiresRequested", s.PitTiresRequested ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.FuelRequested", s.PitFuelRequested ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.FastRepairRequested", s.PitFastRepairRequested ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.WindshieldRequested", s.PitWindshieldRequested ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.FuelDisplay", Escape(s.PitFuelDisplay));
                    Jp(sb, "K10Motorsports.Plugin.PitBox.PressureLF", Escape(s.PitPressureLFDisplay));
                    Jp(sb, "K10Motorsports.Plugin.PitBox.PressureRF", Escape(s.PitPressureRFDisplay));
                    Jp(sb, "K10Motorsports.Plugin.PitBox.PressureLR", Escape(s.PitPressureLRDisplay));
                    Jp(sb, "K10Motorsports.Plugin.PitBox.PressureRR", Escape(s.PitPressureRRDisplay));
                    // Car-specific adjustment availability
                    Jp(sb, "K10Motorsports.Plugin.PitBox.HasTC", s.HasTC ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.HasABS", s.HasABS ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.HasARBFront", s.HasARBFront ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.HasARBRear", s.HasARBRear ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.HasEnginePower", s.HasEnginePower ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.HasFuelMixture", s.HasFuelMixture ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.HasWeightJackerL", s.HasWeightJackerL ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.HasWeightJackerR", s.HasWeightJackerR ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.HasWingFront", s.HasWingFront ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.PitBox.HasWingRear", s.HasWingRear ? 1 : 0);
                    ...
                    Jp(sb, "K10Motorsports.Plugin.DS.EstimatedIRatingDelta", s.EstimatedIRatingDelta);
                    Jp(sb, "K10Motorsports.Plugin.DS.IRatingFieldSize", s.IRatingFieldSize);
                    ...
                    Jp(sb, "K10Motorsports.Plugin.DS.LatG", s.LatAccel, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.LongG", s.LongAccel, ic);
                    ...
                    Jp(sb, "K10Motorsports.Plugin.DS.LicenseString", Escape(s.LicenseString ?? ""));
                    Jp(sb, "K10Motorsports.Plugin.DS.AbsActive", s.AbsActive ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.DS.TcActive", s.TcActive ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.DS.TrackPct", s.TrackPositionPct, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.LapDelta", s.LapDeltaToBest, ic);
                    // Sector splits (legacy 3-sector + N-sector)
                    Jp(sb, "K10Motorsports.Plugin.DS.CurrentSector", s.CurrentSector);
                    Jp(sb, "K10Motorsports.Plugin.DS.SectorCount", s.SectorCount);
                    Jp(sb, "K10Motorsports.Plugin.DS.SectorSplitS1", s.SectorSplitS1, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.SectorSplitS2", s.SectorSplitS2, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.SectorSplitS3", s.SectorSplitS3, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.SectorDeltaS1", s.SectorDeltaS1, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.SectorDeltaS2", s.SectorDeltaS2, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.SectorDeltaS3", s.SectorDeltaS3, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.SectorStateS1", s.SectorStateS1);
                    Jp(sb, "K10Motorsports.Plugin.DS.SectorStateS2", s.SectorStateS2);
                    ...
                    Jp(sb, "K10Motorsports.Plugin.DS.CompletedLaps", s.CompletedLaps);
                    Jp(sb, "K10Motorsports.Plugin.DS.IsInPitLane", s.IsInPitLane ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.DS.SpeedKmh", s.SpeedKmh, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.PitLimiterOn", s.PitLimiterOn ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.DS.PitSpeedLimitKmh", s.PitSpeedLimitKmh, ic);

                    // ── Computed DS.* — server-side calculations ──
                    Jp(sb, "K10Motorsports.Plugin.DS.ThrottleNorm", s.ThrottleNorm, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.BrakeNorm", s.BrakeNorm, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.ClutchNorm", s.ClutchNorm, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.RpmRatio", s.RpmRatio, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.FuelPct", s.FuelPct, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.FuelLapsRemaining", s.FuelLapsRemaining, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.SpeedMph", s.SpeedMph, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.PitSpeedLimitMph", s.PitSpeedLimitMph, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.IsPitSpeeding", s.IsPitSpeeding ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.DS.IsNonRaceSession", s.IsNonRaceSession ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.DS.IsTimedRace", s.IsTimedRace ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.DS.IsEndOfRace", s.IsEndOfRace ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.DS.PositionDelta", s.PositionDelta);
                    Jp(sb, "K10Motorsports.Plugin.DS.StartPosition", s.StartPosition);
                    Jp(sb, "K10Motorsports.Plugin.DS.RemainingTimeFormatted", Escape(s.RemainingTimeFormatted ?? ""));
                    Jp(sb, "K10Motorsports.Plugin.DS.SpeedDisplay", Escape(s.SpeedDisplay));
                    Jp(sb, "K10Motorsports.Plugin.DS.RpmDisplay", Escape(s.RpmDisplay));
                    Jp(sb, "K10Motorsports.Plugin.DS.FuelFormatted", Escape(s.FuelFormatted));
                    Jp(sb, "K10Motorsports.Plugin.DS.FuelPerLapFormatted", Escape(s.FuelPerLapFormatted));
                    Jp(sb, "K10Motorsports.Plugin.DS.PitSuggestion", Escape(s.PitSuggestion ?? ""));
                    Jp(sb, "K10Motorsports.Plugin.DS.BBNorm", s.BBNorm, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.TCNorm", s.TCNorm, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.ABSNorm", s.ABSNorm, ic);
                    Jp(sb, "K10Motorsports.Plugin.DS.PositionDeltaDisplay", Escape(s.PositionDeltaDisplay ?? ""));
                    Jp(sb, "K10Motorsports.Plugin.DS.LapDeltaDisplay", Escape(s.LapDeltaDisplay ?? ""));
                    Jp(sb, "K10Motorsports.Plugin.DS.SafetyRatingDisplay", Escape(s.SafetyRatingDisplay));
                    Jp(sb, "K10Motorsports.Plugin.DS.GapAheadFormatted", Escape(s.GapAheadFormatted));
                    Jp(sb, "K10Motorsports.Plugin.DS.GapBehindFormatted", Escape(s.GapBehindFormatted));
                    ...
                    // ── Commentary ──
                    Jp(sb, "K10Motorsports.Plugin.CommentaryVisible", _engine.IsVisible ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.CommentaryText", Escape(_engine.CurrentText ?? ""));
                    Jp(sb, "K10Motorsports.Plugin.CommentaryTopicTitle", Escape(_engine.CurrentTitle ?? ""));
                    Jp(sb, "K10Motorsports.Plugin.CommentaryTopicId", Escape(_engine.CurrentTopicId ?? ""));
                    ...
                    Jp(sb, "K10Motorsports.Plugin.Demo.Rpm", dt.Rpm, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.MaxRpm", dt.MaxRpm, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.SpeedMph", dt.SpeedMph, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.Throttle", dt.Throttle * 100, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.Brake", dt.Brake * 100, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.Clutch", dt.Clutch * 100, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.Fuel", dt.Fuel, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.MaxFuel", dt.MaxFuel, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.FuelPerLap", dt.FuelPerLap, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.RemainingLaps", dt.RemainingLaps, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.TyreTempFL", dt.TyreTempFL, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.TyreTempFR", dt.TyreTempFR, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.TyreTempRL", dt.TyreTempRL, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.TyreTempRR", dt.TyreTempRR, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.TyreWearFL", dt.TyreWearFL, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.TyreWearFR", dt.TyreWearFR, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.TyreWearRL", dt.TyreWearRL, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.TyreWearRR", dt.TyreWearRR, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.BrakeBias", dt.BrakeBias, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.TC", dt.TC, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.ABS", dt.ABS, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.SessionTypeName", Escape(dt.SessionTypeName ?? ""));
                    ...
                    Jp(sb, "K10Motorsports.Plugin.Demo.RemainingTime", dt.RemainingTime, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.TotalLaps", dt.TotalLaps);
                    Jp(sb, "K10Motorsports.Plugin.Demo.IRating", dt.IRating);
                    Jp(sb, "K10Motorsports.Plugin.Demo.SafetyRating", dt.SafetyRating, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.GapAhead", dt.GapAhead, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.GapBehind", dt.GapBehind, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DriverAhead", Escape(dt.DriverAhead ?? ""));
                    Jp(sb, "K10Motorsports.Plugin.Demo.DriverBehind", Escape(dt.DriverBehind ?? ""));
                    Jp(sb, "K10Motorsports.Plugin.Demo.IRAhead", dt.IRAhead);
                    Jp(sb, "K10Motorsports.Plugin.Demo.IRBehind", dt.IRBehind);

                    // ── Demo Datastream ──
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.LatG", dt.LatG, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.LongG", dt.LongG, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.YawRate", dt.YawRate, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.SteerTorque", dt.SteerTorque, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.TrackTemp", dt.TrackTemp, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.IncidentCount", dt.IncidentCount);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.AbsActive", dt.AbsActive ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.TcActive", dt.TcActive ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.LapDelta", dt.LapDelta, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.IsInPitLane", dt.IsInPitLane ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.SpeedKmh", dt.SpeedKmh, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.PitLimiterOn", dt.IsInPitLane ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.PitSpeedLimitKmh", 72.0, ic);

                    // ── Demo Computed DS.* ──
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.ThrottleNorm", dt.ThrottleNorm, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.BrakeNorm", dt.BrakeNorm, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.ClutchNorm", dt.ClutchNorm, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.RpmRatio", dt.RpmRatio, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.FuelPct", dt.FuelPct, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.FuelLapsRemaining", dt.FuelLapsRemaining, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.SpeedMph", dt.SpeedMph, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.PitSpeedLimitMph", dt.PitSpeedLimitMph, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.IsPitSpeeding", dt.IsPitSpeeding ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.IsNonRaceSession", dt.IsNonRaceSession ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.IsTimedRace", dt.IsTimedRace ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.IsEndOfRace", dt.IsEndOfRace ? 1 : 0);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.PositionDelta", dt.PositionDelta);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.StartPosition", dt.StartPosition);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.RemainingTimeFormatted", Escape(dt.RemainingTimeFormatted ?? ""));
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.SpeedDisplay", Escape(dt.SpeedDisplay));
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.RpmDisplay", Escape(dt.RpmDisplay));
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.FuelFormatted", Escape(dt.FuelFormatted));
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.FuelPerLapFormatted", Escape(dt.FuelPerLapFormatted));
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.PitSuggestion", Escape(dt.PitSuggestion ?? ""));
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.BBNorm", dt.BBNorm, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.TCNorm", dt.TCNorm, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.ABSNorm", dt.ABSNorm, ic);
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.PositionDeltaDisplay", Escape(dt.PositionDeltaDisplay ?? ""));
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.LapDeltaDisplay", Escape(dt.LapDeltaDisplay ?? ""));
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.SafetyRatingDisplay", Escape(dt.SafetyRatingDisplay));
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.GapAheadFormatted", Escape(dt.GapAheadFormatted));
                    Jp(sb, "K10Motorsports.Plugin.Demo.DS.GapBehindFormatted", Escape(dt.GapBehindFormatted));

                    // ── Grid / Formation state ──
                    Jp(sb, "K10Motorsports.Plugin.Grid.SessionState", s.SessionState);
                    Jp(sb, "K10Motorsports.Plugin.Grid.GriddedCars", s.GriddedCars);
                    Jp(sb, "K10Motorsports.Plugin.Grid.TotalCars", s.TotalCars);
                    Jp(sb, "K10Motorsports.Plugin.Grid.PaceMode", s.PaceMode);
                    // Start type: read from iRacing WeekendOptions via shared memory
                    Jp(sb, "K10Motorsports.Plugin.Grid.StartType", Escape(s.IsStandingStart ? "standing" : "rolling"));
                    // Lights phase: synthesized from PaceMode/SessionState transitions
                    Jp(sb, "K10Motorsports.Plugin.Grid.LightsPhase", _lightsPhase);
                    Jp(sb, "K10Motorsports.Plugin.Grid.TrackCountry", Escape(s.TrackCountry ?? ""));

                    // ── Demo Grid state ──
                    Jp(sb, "K10Motorsports.Plugin.Demo.Grid.SessionState", dt.SessionState);
                    Jp(sb, "K10Motorsports.Plugin.Demo.Grid.GriddedCars", dt.GriddedCars);
                    Jp(sb, "K10Motorsports.Plugin.Demo.Grid.TotalCars", dt.TotalCars);
                    Jp(sb, "K10Motorsports.Plugin.Demo.Grid.PaceMode", dt.PaceMode);
                    Jp(sb, "K10Motorsports.Plugin.Demo.Grid.LightsPhase", dt.LightsPhase);
                    Jp(sb, "K10Motorsports.Plugin.Demo.Grid.StartType", Escape(dt.IsStandingStart ? "standing" : "rolling"));
                    Jp(sb, "K10Motorsports.Plugin.Demo.Grid.TrackCountry", Escape(dt.TrackCountry ?? ""));
                    ...
                    if (!string.IsNullOrEmpty(livePlayerName))
                    {
                        ...
                        Jp(sb, "K10Motorsports.Plugin.DriverFirstName", Escape(nameParts[0]));
                        Jp(sb, "K10Motorsports.Plugin.DriverLastName", Escape(nameParts.Length > 1 ? nameParts[1] : ""));
                    }
                    else
                    {
                        Jp(sb, "K10Motorsports.Plugin.DriverFirstName", Escape(Settings.DriverFirstName ?? ""));
                        Jp(sb, "K10Motorsports.Plugin.DriverLastName", Escape(Settings.DriverLastName ?? ""));
                    }
                    ...
                    sb.AppendFormat("\"K10Motorsports.Plugin.Leaderboard\":{0},\n", _leaderboardJson ?? "[]");
                    ...
                    Jp(sb, "K10Motorsports.Plugin.DS.AmbientR", _screenColorSampler.HasColor ? _screenColorSampler.R : 0);
                ...
                {
                    SimHub.Logging.Current.Warn($"[K10Motorsports] HTTP handler error: {ex.Message}");
                }
                finally
                {
