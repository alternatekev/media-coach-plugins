...
export default function AdminPanel() {
  const [tab, setTab] = useState<'tracks' | 'users' | 'logs'>('tracks')
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      <div className="flex gap-1 mb-8 border-b border-[var(--border)]">
        ...
        <TabButton active={tab === 'logs'} onClick={() => setTab('logs')}>Connection Logs</TabButton>
      </div>
      {tab === 'tracks' ? <TracksSection /> : tab === 'users' ? <UsersSection /> : <LogsSection />}
    </div>
  )
}
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-lg font-bold tracking-wide uppercase text-[var(--text-secondary)]">
          API & Database Logs
        </h2>
        <button
          onClick={fetchLogs}
          className="px-3 py-1.5 text-sm bg-[var(--k10-red)] text-white rounded hover:brightness-110 transition-all cursor-pointer"
        >
          Refresh
        </button>
      </div>
            ...
            <tbody>
              {logs.map(log => (
                <tr key={log.id} className="border-t border-[var(--border-subtle)] hover:bg-[var(--bg-elevated)] transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-[var(--text)] font-mono">
                    {log.operation}
                  </td>
                  <td className="px-4 py-3 text-center">
